...
@pytest.fixture
def dns_udp_port(aiomisc_unused_port_factory) -> int:
    return aiomisc_unused_port_factory()


@pytest.fixture
def dns_tcp_port(aiomisc_unused_port_factory) -> int:
    return aiomisc_unused_port_factory()


@pytest.fixture
def dns_server_udp(dns_store_filled: DNSStore, dns_udp_port) -> UDPDNSServer:
    return UDPDNSServer(
        store=dns_store_filled, address="localhost", port=dns_udp_port
    )


@pytest.fixture
def dns_server_tcp(dns_store_filled: DNSStore, dns_tcp_port) -> TCPDNSServer:
    return TCPDNSServer(
        store=dns_store_filled, address="localhost", port=dns_tcp_port
    )
...
async def test_handle_datagram_a_record(services, dns_udp_port):
    query = dnslib.DNSRecord.question("sub.example.com.", qtype="A")
    query_data = query.pack()

    response = await dns_send_receive_udp(query_data, dns_udp_port)
...
async def test_handle_datagram_aaaa_record(services, dns_udp_port):
...
async def test_handle_datagram_cname_record(services, dns_udp_port):
...
async def test_handle_datagram_nonexistent_record(services, dns_udp_port):
...
async def test_handle_datagram_remove_record(
    services, dns_store_filled, dns_udp_port
):
...
async def test_handle_datagram_edns_record(services, dns_udp_port):
    query = dnslib.DNSRecord.question("sub.example.com.", qtype="A")
...
async def test_handle_large_datagram_truncated_udp(
    services, dns_udp_port, dns_store_filled
):
    zone = dns_store_filled.get_zone("example.com.")
...
async def test_handle_large_tcp_request(
    services, dns_tcp_port, dns_store_filled
):
    zone = dns_store_filled.get_zone("example.com.")
    for i in range(1, 101):
    ...
    response = await dns_send_receive_tcp(query_data, dns_tcp_port)
